...
//https://docs.google.com/forms/d/1rshQNhVufch_FR4iPCTyk0PDRag1Xw9jWVlZT2tY294/viewform?entry.554476735=1&entry.279674121=South+Street+%26+Whitehall+Street&entry.986802208&entry.622241787&entry.381623989
    ...
    that.zipcodeElements = that.activityLayer.selectAll("circle.zipcode")
                              ...
                              .attr("class", function(d) {
                                if (d.properties.assigned < d.properties.capacity) {
                                  return "grassroots-event";
                                } else {
                                  return "official-rally";
                                }
                              })
                              .attr("r", function(d) {
  ...
  this.replot = function () {
    ...
    that.zipcodeElements.each(function(d) {
      var coordinates = that._projectPoint(d.geometry.coordinates[0], d.geometry.coordinates[1]);
        ...
        d3.select(this).attr("cx", coordinates[0])
            .attr("cy", coordinates[1])
            .attr("r", bernMap.mapBox.getZoom() * 0.8)
              // function (d) {
              //   return bernMap.scale.radScale(d.properties.attendee_count);
              // })
            .attr("opacity", 0.6)
...
bernMap.eventList = function(container) {
  ...
  this.buildEvent = function(d) {
     ...
     if (d.properties.capacity > d.properties.assigned) {
      eventText = "Open";
      eventType = "meetup";
     } else {
      eventText = "Full";
     ...
     if (d.properties.assigned >= d.properties.capacity && d.properties.capacity > 0) {
          ...
          return "<h5><span class='event-item-date'>"
            + (!isNaN(d.properties.distance) ? ("~" + d3.round(d.properties.distance,1) + "MI&nbsp;&nbsp; ") : "")
            + "</span></h5>"
            + "<h3><a target='_blank' href='" + d.properties.link + "'><span class='event-item-name'>" + d.properties.station + " Station</span></a></h3>"
            + "<div class='event-type " + eventType + "'><span class='event-bullet'>&bull;</span><span class='event-text'>" + eventText + "</span></div>"
            // + (d.properties.description != "" ? ("<h4 class='event-organizer'>" + d.properties.description +"</h4>") : "")
            + "<h5 class='event-location'>" + d.properties.address + " New York, NY</h5>"
            + "<p><a href='" + d.properties.link + "' target='_blank' class='button-rsvp'>CANVAS HERE " + d.properties.assigned + "/" + d.properties.capacity+ "</a> "
...
// d3.json("./csv-grab.php?u=" + encodeURIComponent(bernMap.constants.spreadsheetUrl),
  ...
  window.dataCallback = function(){
    ...
    $(bernMap.d.meetupData).each(function(i, item) {
      ...
      // var tempTime = rawTime.parse(item.starttime);
      ...
      // item.OrganizerWebsite = item.organizer_url;
...
$jq(window).on("hashchange", function(){
  ...
  if (hash.length > 1) {
    ...
    if(bernMap.d.allZipcodes){
      ...
      bernieEvents.filterEvents(parameters.zipcode, 10);
    }
